// semaforos.c
// Compilar con: gcc semaforos.c -o semaforos -lpthread -lm
...
void *proceso(void *datos) {
    ...
    int a = datos_proceso->dato;
    int p = datos_proceso->p;
...
int main() {
    ...
    error = pthread_create(&idhilo1, NULL, proceso, &hilo1_datos);
    if (error != 0) {
        perror("No puedo crear hilo");
        exit(-1);
    }

    error = pthread_create(&idhilo2, NULL, proceso, &hilo2_datos);
    if (error != 0) {
